...
<body>';
   ...
   static function addslashes($string, $force = 1) {
     ...
     * 时间转化为字符串
     * */
    static function subtimer($timer) {
        $subtimer = time() - $timer;
        if($subtimer > 3600 * 216) {
            $timestring =  date('Y-m-d', $timer);
            return $timestring;
     ...
     * 返回，数组
     * */
    static function getYearArray($from = 0, $to = 0) {
        if($from == 0) $from = date('Y', time());
        if($to == 0) $to = 1949;
     ...
     * 返回，数组
     * */
    static function getMonthArray($prefix = true) 
    {
        $Months = array();
        
        if($prefix) {
            for($i = 1; $i<=12; $i++) $Months[$i] = sprintf("%02d", $i);
        } else {
