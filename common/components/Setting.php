...
class Setting extends Component
{
    ...
    public function set($date)
    {
        foreach ($date as $key => $value) {
            Yii::$app->db->createCommand()->update('{{%admin_config}}', [
                'config' => $value
            ], '`name`=:key', [':key' => $key])->execute();
        }
    ...
    {
        $settings = Yii::$app->db->createCommand("SELECT * FROM {{%admin_config}}")->queryAll();
        return \yii\helpers\ArrayHelper::map($settings, 'name', 'config');
    }
}
