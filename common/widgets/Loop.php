...
class Loop extends Widget{
	...
	public function init(){
		if($this->data === null){
			throw new InvalidConfigException('TLoop::data or tableName or catalogId must be set.');
		}
		ob_start();
	...
	public function run()
	{
		...
		if(! empty($this->item)){
			...
			if(strpos($this->item, '@') === 0 || strpos($this->item, '/') === 0){
				...
				$itemTemplate = '@frontend/widgets/views/loop/' . $this->item;
			}
